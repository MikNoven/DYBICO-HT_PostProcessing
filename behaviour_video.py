...
##############Helper functions###################
def getTimeWindow(start_indx,displayWindow,time):
    tw=[0,0]
...
for day_nbr in range(1,len(HT_days)+1):
    ...
    for session in range(1,len(sessionFiles)+1):
        ...
        video_folder = os.path.join(report_dir,subj,'Day_1','Session_'+str(session)+'_video')
        ...
        #Chande codec
        video = cv2.VideoWriter(os.path.join(video_folder,'Session_'+str(session)+'.mp4'), video.fourcc('M','P','4','V'), 1/(timeCoarsness*dt), (width,height))
