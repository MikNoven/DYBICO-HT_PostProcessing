...
def day_handsep_plot(print_data_means, print_data_std, plot_yaxis, plot_title, save_destination):
    ...
    #ax.set_ylim((0,100))
    ax.set_xticks(x_pos)
    ax.set_xticklabels(['Run 1', 'Run 2', 'Run 3', 'Run 4'], fontsize=16, fontweight='bold')
    ax.set_title(plot_title, fontsize=16, fontweight='bold')
    ax.yaxis.grid(True)
...
def fMRI_pdf_report_plots(path,report_dir,subj):
    overwrite = True
    ...
    handsep_bas_std = [None]*len(HT_days)
    ...
    for day_itr in range(len(HT_days)): #Check if it's not already done and if overwrite is false
        ...
        ToT_bas_L[day_itr] = np.mean(np.asarray(block_ToT_bas_L))
        ToT_bas_R[day_itr] = np.mean(np.asarray(block_ToT_bas_R))
        handsep_bas[day_itr] = np.mean(np.asarray(block_handsep_bas))
        overshoot_L_bas[day_itr] = np.mean(np.mean(np.asarray(block_overshoot_L_bas),axis=1),axis=0)
        overshoot_R_bas[day_itr] = np.mean(np.mean(np.asarray(block_overshoot_R_bas),axis=1),axis=0)
        undershoot_L_bas[day_itr] = np.mean(np.mean(np.asarray(block_undershoot_L_bas),axis=1),axis=0)
        undershoot_R_bas[day_itr] = np.mean(np.mean(np.asarray(block_undershoot_R_bas),axis=1),axis=0)
        volatility_L_bas[day_itr] = np.mean(np.mean(np.asarray(block_volatility_L_bas),axis=1),axis=0)
        volatility_R_bas[day_itr] = np.mean(np.mean(np.asarray(block_volatility_R_bas),axis=1),axis=0)

        ToT_bas_L_std[day_itr] = np.std(np.asarray(block_ToT_bas_L))
        ToT_bas_R_std[day_itr] = np.std(np.asarray(block_ToT_bas_R))
        handsep_bas_std[day_itr] = np.std(np.asarray(block_handsep_bas))
        overshoot_L_bas_std[day_itr] = np.std(np.std(np.asarray(block_overshoot_L_bas),axis=1),axis=0)
        overshoot_R_bas_std[day_itr] = np.std(np.std(np.asarray(block_overshoot_R_bas),axis=1),axis=0)
        undershoot_L_bas_std[day_itr] = np.std(np.std(np.asarray(block_undershoot_L_bas),axis=1),axis=0)
        undershoot_R_bas_std[day_itr] = np.std(np.std(np.asarray(block_undershoot_R_bas),axis=1),axis=0)
        volatility_L_bas_std[day_itr] = np.std(np.std(np.asarray(block_volatility_L_bas),axis=1),axis=0)
        volatility_R_bas_std[day_itr] = np.std(np.std(np.asarray(block_volatility_R_bas),axis=1),axis=0)
    ...
    tot_plots_per_page_2 = ['MaxForces.png', 'Tot_ToT_B.png', 'Tot_vol_1of3_B.png', 'Tot_vol_2of3_B.png', 'Tot_vol_3of3_B.png']
