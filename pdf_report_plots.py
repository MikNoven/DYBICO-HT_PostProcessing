...
for subj in list_of_subjs:
    ...
    for day_itr in range(len(HT_days)): #Check if it's not already done and if overwrite is false
        ...
        if not os.path.exists(os.path.join(day_save_folder,'Day_Succ_A.png')) or overwrite:
            ...
            day_trialsuccess_plot([x for x in sess_trialSuccess_asym_both], [x for x in sess_std_trialSuccess_asym_both], [x for x in sess_trialSuccess_asym_L], [x for x in sess_std_trialSuccess_asym_L], [x for x in sess_trialSuccess_asym_R], [x for x in sess_std_trialSuccess_asym_R], 'Successful trials / %', 'Successfully stable (<=0.5% max force for 200 ms)\nin Asymmetric conditions', os.path.join(day_save_folder,'Day_Succ_A.png'))
            day_trialsuccess_plot([x for x in sess_trialSuccess_sym_both], [x for x in sess_std_trialSuccess_sym_both], [x for x in sess_trialSuccess_sym_L], [x for x in sess_std_trialSuccess_sym_L], [x for x in sess_trialSuccess_sym_R], [x for x in sess_std_trialSuccess_sym_R], 'Successful trials / %', 'Successfully stable (<=0.5% max force for 200 ms)\nin Symmetric conditions', os.path.join(day_save_folder,'Day_Succ_S.png'))
    ...
    if not ToT_asym_L[0]==None:
        ...
        #Make PA table
        plt.ioff()
        PA_df = pd.DataFrame(data=physical_activity_matrix, columns=['TrainingDay', 'Walk', 'Cardio', 'Swimming', 'Weight Training', 'Other'])
        fig, ax = plt.subplots()
        ax.axis('off')
        ax.axis('tight')
